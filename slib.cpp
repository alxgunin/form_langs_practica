...
void NFA::ParseAndMakeGraph(string regexp) {
    ...
    for (char cur : regexp) {
        ...
        case '+':
            if (stack.size() < 2) {
                string error = "Too few argumets for operation \"+\", expected 2, got " + std::to_string(stack.size());
        ...
        case '*':
            if (stack.size() < 1) {
                throw std::runtime_error("Too few argumets for operation \"*\", expected 1, got 0");
            }
            begin1 = stack.top().first;
            end1 = stack.top().second;
            stack.pop();
